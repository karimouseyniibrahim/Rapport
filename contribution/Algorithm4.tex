...
\SetKwFunction{printlcs}{Envoi de:}
\begin{algorithm}[H]
 \SetAlgoLined\DontPrintSemicolon
  \SetKwFunction{algo}{Envoi de: }\SetKwFunction{proc}{proc}
  \SetKwProg{myalg}{Procedure}{}{}
  \myalg{\algo{$id,\parametretwo{},\parametretree{},\parametrefour{},f$}par la la machine i}{
  	\Begin{  \nl\ForEach{$e\in \{Notifier(S)\cup Border(S)\}$}{ \label{linesend1}
   		\uIf{$(\curenti{} \in e.i)$ }{\label{linesend2}
   			$F=(type(e)==Notifier) ? e : \emptyset$\;\label{linesend3}
    		 	\ForEach{$m \in e.site $}{\label{linesend4}
   				$send(e.id,e.\parametretwo{},e.\parametretree{},F,e.\parametrefour{},i)\;\;  to \;\;m.id )$\;
   			}\label{linesend5}
    		}
    	}  
   }  	
...
\SetKwFunction{printlcs}{Reception de:}
\begin{algorithm}
 \SetAlgoLined\DontPrintSemicolon
  \SetKwFunction{algo}{\printlcs{$id,\parametretwo{},\parametretree{},\parametrefour{},f,i$} par la machine j}\SetKwFunction{proc}{proc}
  \SetKwProg{myalg}{Procedure}{}{}
  \myalg{\algo{}}{
  	\Begin{
  \nl\ForEach{$e\in \; S$}{ \label{linereceve1}
   		\If{$(e.id==id )$ }{\label{linereceve2}
   			$x.site[i].\parametretwo{} \longleftarrow \parametretwo{}$\;
   			$x.site[i].\parametretree{} \longleftarrow \parametretree{}$ \;
   			$x.site[i].\parametrefour{} \longleftarrow \parametrefour{}$\;
   			$x.site[i].f\longleftarrow f$\;
   			}
    		} 	
    	}
}{}
...
\begin{Exemple}\label{ea4}
	...
	\item[Itération 2]
		\begin{itemize}
			\item A la réception des valeurs: id =s9 ;$\parametretwo{}$ =6 ;$\parametretree{}$ =0 ;$\parametrefour{}$=0 ;f =Ø, i=1 par la \mtwo{}. Ces dernières sont stockées à l'indice \s{i} de la variable  \s{site} de l'état \s{S9}.
